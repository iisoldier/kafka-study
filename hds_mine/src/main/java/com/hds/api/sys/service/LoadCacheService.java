 ...
 */
    ...
    * @return  void
	...
	public ReloadCacheResponse reloadCache(ReloadCacheRequest rcrqVo,BindingResult result,
		...
		//获取本接口API 标志常量
	    String api_id = R.APIConstant.API_SYS_Reload_Cache;
		...
		if(list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.PARAM_IS_VALID)&&
		   ...
		   rr_req_frequency.getResult_code().equals(RequestValidation.ResultCode.FREQUENCY_IS_VALID)){	
			   ...
			   if(code == 1){
					...
					rr.setResult_code(QueryHandleConstant.ResultCode.RELOAD_CACHEMAP_SUCCESS);
					rr.setField(QueryHandleConstant.Field.RELOAD_CACHEMAP);
					rr.setMessage(QueryHandleConstant.ResultMessage.RELOAD_CACHEMAP_SUCCESSMSG);
					list_rr.add(rr);
			   }
			   if(code ==2){
					ResponseResult rr = new ResponseResult();
			   ...
			   if(code ==0 ){
					ResponseResult rr = new ResponseResult();
					rr.setType(R.SystemConstant.RESTYPE_HANDLE);
					rr.setResult_code(QueryHandleConstant.ResultCode.RELOAD_CACHEMAP_ERROR);
					rr.setField(QueryHandleConstant.Field.RELOAD_CACHEMAP);
					rr.setMessage(QueryHandleConstant.ResultMessage.RELOAD_CAHCEMAP_ERRORMSG);
					list_rr.add(rr);
			   }
