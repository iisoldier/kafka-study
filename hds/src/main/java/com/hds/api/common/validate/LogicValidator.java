	...
	* @return  ResponseResult
	...
	public static ResponseResult DateValidate(String startdate,String enddate,int day_lag, String request_id){
	   ...
	   if(days_between>day_lag){
		   ...
		   rr.setResult_code(LogicValidation.ResultCode.LOGIC_DAYS_LAG);
		   rr.setField(LogicValidation.Field.LOGIC_SDAY_EDAY);
		   rr.setMessage(LogicValidation.ValidateMessage.DAYS_LAG_ERRMSG+
	   ...
	   if(days_between<0){
		  ...
		  ResponseResult rr = new ResponseResult();
		  rr.setType(R.SystemConstant.RESTYPE_LOGIC);
		  rr.setResult_code(LogicValidation.ResultCode.LOGIC_DAYS_COMPARE);
		   rr.setField(LogicValidation.Field.LOGIC_SDAY_EDAY);
		   rr.setMessage(LogicValidation.ValidateMessage.DAYS_COMPARE_ERRMSG+
				  ",输入参数为|"+startdate+"|"+enddate+"|"+days_between);
		...
		ResponseResult rr = new ResponseResult();
		rr.setType(R.SystemConstant.RESTYPE_LOGIC);
		rr.setResult_code(LogicValidation.ResultCode.LOGIC_IS_VALID);
		rr.setField(LogicValidation.Field.LOGIC_SDAY_EDAY);
		rr.setMessage(LogicValidation.ValidateMessage.LOGIC_VALID_SUCESSMSG);
	    //debug日志(可以删除)
