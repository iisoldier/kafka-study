	...
	public  static List<ResponseResult> authValidate(String  sys_id , String api_id ,String request_ip ,String request_id) {
		...
		for (int i = 0; i < syslist.size(); i++) {
			...
			if(sys_id.equals(syslist.get(i).getSys_id())&&
					RequestValidation.Constant.SYSACCESS_OPEN_FLAG==syslist.get(i).getSys_flag()){
				//debug日志(可删除)
		...
		if(count_syslist==0){		
			...
			rr.setResult_code(RequestValidation.ResultCode.AUTH_SYS_INVALID);
			rr.setField(RequestValidation.Field.AUTH_SYS_IDFLAGE);
			rr.setMessage(RequestValidation.ValidateMessage.AUTH_SYS_NOTEXIST_ERRMSG+"后台常量设置为:"+sys_id);
			Ist_rr.add(rr);
			//debug日志(可删除)
		...
		for (int i = 0; i < apilist.size(); i++) {
			//校验api开关是否处于开启状态
			if(api_id.equals(R.APIEnum.valueOf(api_id).getValue())&&
					RequestValidation.Constant.API_OPEN_FLAG == apilist.get(i).getApi_flag()){
				//debug日志(可删除)
		...
		if(count_syslist==1&&count_apilist==0){
			ResponseResult rr = new ResponseResult();			
			rr.setType(R.SystemConstant.RESTYPE_AUTH);
			rr.setResult_code(RequestValidation.ResultCode.AUTH_API_INVALID);
			rr.setField(RequestValidation.Field.AUTH_API_IDFLAG);
			rr.setMessage(RequestValidation.ValidateMessage.AUTH_API_NOTEXIST_ERRMSG+",后台常量设置为:"+api_id);
			Ist_rr.add(rr);
			//debug日志(可删除)
		...
		if(count_syslist==1&&count_apilist==1&&count_ipwhitelist==0){
			ResponseResult rr = new ResponseResult();
			rr.setType(R.SystemConstant.RESTYPE_AUTH);
			rr.setResult_code(RequestValidation.ResultCode.AUTH_SYSIP_INVALID);
			rr.setField(RequestValidation.Field.AUTH_SYS_IP);
			rr.setMessage(RequestValidation.ValidateMessage.AUTH_SYSIP_NOTACCESS_ERRMSG+",后台常量设置为:"+sys_id+"|"+request_ip);
			Ist_rr.add(rr);
			//debug日志(可删除)
	    ...
	    if(count_syslist==1&&count_apilist==1&&count_ipwhitelist==1&&count_apisyslist==0){
			ResponseResult rr = new ResponseResult();
			rr.setType(R.SystemConstant.RESTYPE_AUTH);
			rr.setResult_code(RequestValidation.ResultCode.AUTH_APISYS_INVALID);
			rr.setField(RequestValidation.Field.AUTH_API_SYS);
			rr.setMessage(RequestValidation.ValidateMessage.AUTH_SYSAPI_NOTACCESS_ERRMSG+",后台常量设置为:"+sys_id+"|"+api_id);
			Ist_rr.add(rr);
			//debug日志(可删除)
	   ...
	   if(count_syslist ==1 && count_apilist ==1 && count_ipwhitelist==1 && count_apisyslist==1){
			ResponseResult rr = new ResponseResult();
			rr.setType(R.SystemConstant.RESTYPE_AUTH);
			rr.setResult_code(RequestValidation.ResultCode.AUTH_IS_VALID);
			rr.setField(RequestValidation.Field.AUTH_IS_VALID);
			rr.setMessage(RequestValidation.ValidateMessage.AUTH_VALID_SUCESSMSG);
			Ist_rr.add(rr);
			//debug日志(可删除)
