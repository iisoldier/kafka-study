...
public class CCSDemoServiceImpl implements CCSDemoService {
	...
	* @return  CCSDemoResponseVo
	...
	public CCSDemoResponseVo getCCSDemo(CCSDemoRequestVo ccsVo,BindingResult result,
		...
		// 获取本接口API 标志常量
		String api_id = R.APIConstant.API_CCS_GetCCSDemo;
		...
		ResponseResult rr_date = LogicValidator.DateValidate(
				...
				RequestValidation.Constant.DAYS_LAG_CCSDEMO,
				ccsVo.getRequest_id());
		// 参数校验成功，参数逻辑校验失败
		if (list_authrr.get(0).getResult_code().equals(RequestValidation.ResultCode.AUTH_IS_VALID)
				&& rr_req_frequency.getResult_code().equals(RequestValidation.ResultCode.FREQUENCY_IS_VALID)
				&& list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.PARAM_IS_VALID)
		...
		if (list_authrr.get(0).getResult_code().equals(RequestValidation.ResultCode.AUTH_IS_VALID)
			...
			rr.setResult_code(RequestValidation.ResultCode.ALL_IS_VALID);
			rr.setField(RequestValidation.Field.FIELD_ALL_VALID);
			rr.setMessage(RequestValidation.ValidateMessage.ALL_VALID_SUCCESSMSG);
			list_rr.add(rr);
