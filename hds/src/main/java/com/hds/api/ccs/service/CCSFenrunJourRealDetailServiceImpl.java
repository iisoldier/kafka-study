...
import com.hds.api.ccs.dao.CCSFenrunJourRealDetailDao;
...
/**
* @ClassName:com.hds.api.ccs.service.CCSFenrunJourServiceImpl
* @Description:分润流水查询接口
* @author zengli
* @date 创建时间：2016年4月26日 下午4:40:29
 */
@Service("CCSFenrunJourRealDetailService")
public class CCSFenrunJourRealDetailServiceImpl implements CCSFenrunJourRealDetailService {

	private static Logger logger = Logger.getLogger(CCSFenrunJourRealDetailServiceImpl.class);

	
  @Autowired
   private CCSFenrunJourRealDetailDao ccsFenrunJourRealDetailDao;
	
	

	/**
	* @method  getCCSFeeSettleOrder
	* @Description: getCCSFeeSettleOrder 接口实现方法  用于划付指令查询
	* @author  zengli
	...
	* @return  FeeSettleOrderResponse
	...
	public FenrunJourRealDetailResponse getFenrunJourRealDetail(FenrunJourRealDetailRequest fjrdVo,
		...
		//获取本接口API 标志常量
		String api_id = R.APIConstant.API_CCS_GetFenrunJourRealDetail;
       ...
       //如果常量信息错误，则说明有有问题
		...
		List<ResponseResult> list_rprr = RequestParamValidator
				.requestParamValidate(result, fjrdVo.getRequest_id());
		//debug日志(可去除)
		logger.info("参数格式校验返回结果数量:" + list_rprr.size());
		//若返回结果为空,抛出异常,避免空指针
		if (list_rprr.size() == 0) {
			logger.error(fjrdVo.getRequest_id()+"|"+fjrdVo.getRequest_ip()+"|"+
		...
		if (!list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.PARAM_IS_VALID)) {
			...
			// 创建查询返回对象
			FenrunJourRealDetailResponse respvo = new FenrunJourRealDetailResponse();
			respvo.setRequest_id(fjrdVo.getRequest_id());
			respvo.setSysid(fjrdVo.getSysid());
			respvo.setResponsetime(DateUtils.toString(R.DateConstant.DATETIME_FULL));
			respvo.setRequest_param(fjrdVo);
			respvo.setResponse_records(null);
			respvo.setReponse_result(list_rprr);
			//打印日志
			logger.info("request_id:"+fjrdVo.getRequest_id()+"请求校验失败,"
		...
		List<ResponseResult> list_authrr = AuthValidator.authValidate(
				fjrdVo.getSysid(), api_id, fjrdVo.getRequest_ip(),
				fjrdVo.getRequest_id());
		//debug日志(可去除)
		...
		ResponseResult rr_date = LogicValidator.
				DateValidate(fjrdVo.getS_core_trade_date(),fjrdVo.getE_core_trade_date(),
						RequestValidation.Constant.DAYS_LAG_CCSFENRUNJOURREALDETAIL,
						fjrdVo.getRequest_id());
		...
		if ( list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.PARAM_IS_VALID)
			...
			FenrunJourRealDetailResponse respvo = ccsFenrunJourRealDetailDao.getFenrunJourRealDetail(fjrdVo, ots_config);
			...
			//打印日志
			logger.info("request_id:"+fjrdVo.getRequest_id()+"请求校验成功,"
					+"返回信息为:["+ JSON.toJSONString(respvo)+"]");
			return respvo;
		}
		// --------------------  请求校验全部成功结束------------------------
		...
		respvo.setReponse_result(list_rprr);
		//异常处理情况
		logger.error("request_id:"+fjrdVo.getRequest_id()+"请求校验失败,"
				+"返回信息为:["+ JSON.toJSONString(respvo)+"]");
		return respvo;
		// --------------------  异常校验状态处理 结束 ------------------------
	}		
