...
import com.hds.api.ccs.dao.CCSFenrunJourDao;
 ...
 */
@Service("CCSFenrunJourService")
public class CCSFenrunJourServiceImpl implements CCSFenrunJourService {

	private static Logger logger = Logger.getLogger(CCSFenrunJourServiceImpl.class);

	
  @Autowired
   private CCSFenrunJourDao ccsFenrunJourDao;
	...
	* @return  FeeSettleOrderResponse
	...
	public FenrunJourResponse getFenrunJour(FenrunJourRequest fjVo,
		...
		//获取本接口API 标志常量
		String api_id = R.APIConstant.API_CCS_GetFenrunJour;
       ...
       //如果常量信息错误，则说明有有问题
		...
		if (!list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.PARAM_IS_VALID)) {
			...
			// 创建查询返回对象
			FenrunJourResponse respvo = new FenrunJourResponse();
		...
		ResponseResult rr_date = LogicValidator.
				DateValidate(fjVo.getS_occur_date(),fjVo.getE_occur_date(),
						RequestValidation.Constant.DAYS_LAG_CCSFENRUNJOUR,
						fjVo.getRequest_id());
		...
		if ( list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.PARAM_IS_VALID)
			...
			FenrunJourResponse respvo = ccsFenrunJourDao.getFenrunJour(fjVo, ots_config);
