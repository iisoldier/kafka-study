...
import com.hds.api.ccs.service.CCSFenrunJourService;
...
/**
* @ClassName:com.hds.api.ccs.controller.CCSFenrunJourController
* @Description:分润流水查询控制器
* @author zengli
* @date 创建时间：2016年4月26日 下午3:00:39
 */
@Controller
@RequestMapping("/ccs")
public class CCSFenrunJourController {

	
	private static Logger logger = Logger.getLogger(CCSFenrunJourController.class);
		
	//计费系统服务Service
	@Resource
	CCSFenrunJourService  ccsFenrunJourService;
	...
	* @return  FeeSettleOrderResponse
	 */
	@RequestMapping(value="/getFenrunJour")
	@Frequency(name=R.APIConstant.API_CCS_GetFenrunJour,limit=1,time=3)
	public @ResponseBody FenrunJourResponse getFenrunJour(HttpServletRequest request,
			 @ModelAttribute("fjVo")@Valid FenrunJourRequest fjVo,
		...
		logger.info(request_id+R.SystemConstant.LOG_OBJECT_DELIMITER+ 
				    ...
				    R.APIConstant.API_CCS_GetFenrunJour
				    +R.SystemConstant.LOG_OBJECT_DELIMITER+fjVo.toString()
				    +R.SystemConstant.LOG_OBJECT_DELIMITER+rr_req_frequency.toString());
		
		
		//调用Service服务
		FenrunJourResponse  fjrVo = ccsFenrunJourService.getFenrunJour(fjVo, result, rr_req_frequency);
