...
import com.hds.api.ccs.dao.CCSFeeSettleOrderDao;
 ...
 */
@Service("CCSFeeSettleOrderService")
public class CCSFeeSettleOrderServiceImpl implements CCSFeeSettleOrderService {

	private static Logger logger = Logger.getLogger(CCSFeeSettleOrderServiceImpl.class);

	
  @Autowired
   private CCSFeeSettleOrderDao ccsFeeSettleOrderDao;
	...
	* @return  FeeSettleOrderResponse
	...
	public FeeSettleOrderResponse getFeeSettleOrder(FeeSettleOrderRequest fsoVo,
		...
		//获取本接口API 标志常量
		String api_id = AuthValidation.APIConstant.API_CCS_GETFEESETTLEORDER;
       ...
       //如果常量信息错误，则说明有有问题
		...
		List<ResponseResult> list_rprr = RequestParamValidator
				.requestParamValidate(result, fsoVo.getRequest_id());
		//debug日志(可去除)
		logger.info("参数格式校验返回结果数量:" + list_rprr.size());
		//若返回结果为空,抛出异常,避免空指针
		if (list_rprr.size() == 0) {
			logger.error(fsoVo.getRequest_id()+"|"+fsoVo.getRequest_ip()+"|"+
		...
		if (!list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.REQUEST_PARAM_VALID)) {
			...
			//打印日志
			logger.info("request_id:"+fsoVo.getRequest_id()+"请求校验失败,"
		...
		List<ResponseResult> list_authrr = AuthValidator.authValidate(
				fsoVo.getSysid(), api_id, fsoVo.getRequest_ip(),
				fsoVo.getRequest_id());
		//debug日志(可去除)
		...
		ResponseResult rr_date = LogicValidator.
				DateValidate(fsoVo.getS_occur_date(), fsoVo.getE_occur_date(),
						LogicValidation.Constant.CCSFEESETTLEORDER_DAYS_LAG,
						fsoVo.getRequest_id());
		// 请求参数格式-请求权限-请求频率校验成功，请求参数逻辑校验失败
		if (list_rprr.get(0).getResult_code().equals(RequestValidation.ResultCode.REQUEST_PARAM_VALID)
			&&list_authrr.get(0).getResult_code().equals(AuthValidation.ResultCode.AUTH_IS_VALID)
			&& rr_req_frequency.getResult_code().equals(RequestValidation.ResultCode.FREQUENCY_IS_VALID)
            && !rr_date.getResult_code().equals(LogicValidation.ResultCode.LOGIC_IS_VALID)){
		...
		if (list_authrr.get(0).getResult_code().equals(AuthValidation.ResultCode.AUTH_IS_VALID)
			...
			FeeSettleOrderResponse respvo = ccsFeeSettleOrderDao.getFeeSettleOrder(fsoVo, ots_config);
			...
			//打印日志
			logger.info("request_id:"+fsoVo.getRequest_id()+"请求校验成功,"
					+"返回信息为:["+ JSON.toJSONString(respvo)+"]");
			return respvo;
		}
		// --------------------  请求校验全部成功结束------------------------
		...
		respvo.setReponse_result(list_rprr);
		//异常处理情况
		logger.error("request_id:"+fsoVo.getRequest_id()+"请求校验失败,"
				+"返回信息为:["+ JSON.toJSONString(respvo)+"]");
		return respvo;
		// --------------------  异常校验状态处理 结束 ------------------------
	}		
