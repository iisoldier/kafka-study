	...
	alter table  f_his_fee_settle_order drop if     exists partition (dt = '$date_dt');
	alter table  f_his_fee_settle_order add  if not exists partition (dt = '$date_dt');

	insert into table  f_his_fee_settle_order partition( dt = '$date_dt' )
	select
