	...
	alter table  f_his_ccs_fenrun_real drop if     exists partition (dt = '$date_dt');
	alter table  f_his_ccs_fenrun_real add  if not exists partition (dt = '$date_dt');

	insert into table  f_his_ccs_fenrun_real partition( dt = '$date_dt' )
	select
