	...
	alter table  f_his_ccs_fee_jour drop if     exists partition (dt = '$date_dt');
	alter table  f_his_ccs_fee_jour add  if not exists partition (dt = '$date_dt');

	insert into table  f_his_ccs_fee_jour partition( dt = '$date_dt' )
	select
		
		case when fee_sn 	 	 	is null then '' else fee_sn end,
		case when fee_id 	 	 	is null then '' else fee_sn end,
		case when fee_name 	 		is null then '' else fee_sn end,
		case when fee_type_flag 	is null then '' else fee_sn end,
		case when serial_no 	 	is null then '' else fee_sn end,
