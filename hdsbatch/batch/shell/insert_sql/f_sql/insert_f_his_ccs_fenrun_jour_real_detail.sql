	...
	alter table  f_his_ccs_fenrun_jour_real_detail drop if     exists partition (dt = '$date_dt');
	alter table  f_his_ccs_fenrun_jour_real_detail add  if not exists partition (dt = '$date_dt');

	insert into table  f_his_ccs_fenrun_jour_real_detail partition( dt = '$date_dt' )
	select
		
		case when fenrun_fee_sn is null then '' else fenrun_fee_sn end,
		case when fenrun_fee_id is null then '' else fenrun_fee_id end,
		case when fenrun_fee_name is null then '' else fenrun_fee_name end,
		case when fenrun_fee_type_flag is null then '' else fenrun_fee_type_flag end,
		case when fenrun_serial_no is null then '' else fenrun_serial_no end,
		...
		case when length(trim(fenrun_occur_date))==10 then concat(concat(substr(fenrun_occur_date,1,4),substr(fenrun_occur_date,6,2)),substr(fenrun_occur_date,9,2)) 
			when fenrun_occur_date is null then '' else fenrun_occur_date end,
		case when fenrun_org_code is null then '' else fenrun_org_code end,
		case when fenrun_org_name is null then '' else fenrun_org_name end,
		case when fenrun_currency is null then '' else fenrun_currency end,
		case when fenrun_fee_amount is null then '' else fenrun_fee_amount end,
		case when fenrun_fee_cycle_id is null then '' else fenrun_fee_cycle_id end,
		case when fenrun_fee_detail_id is null then '' else fenrun_fee_detail_id end,
		case when fenrun_create_type is null then '' else fenrun_create_type end,
		case when fenrun_last_update_date is null then '' else fenrun_last_update_date end,
		case when fenrun_last_update_time is null then '' else fenrun_last_update_time end,
		case when fenrun_gather_flag is null then '' else fenrun_gather_flag end,
		case when fenrun_need_gather is null then '' else fenrun_need_gather end,
		case when fenrun_channel_serial_no is null then '' else fenrun_channel_serial_no end,
		case when fenrun_create_date is null then '' else fenrun_create_date end,
		case when fenrun_result_code is null then '' else fenrun_result_code end,
		case when fenrun_result_info is null then '' else fenrun_result_info end,
		case when fenrun_detail_no is null then '' else fenrun_detail_no end,
		...
		case when length(trim(core_trade_date))==10 then concat(concat(substr(core_trade_date,1,4),substr(core_trade_date,6,2)),substr(core_trade_date,9,2)) 
					when core_trade_date is null then '' 
					else core_trade_date end,
	...
	from 
	 o_his_ccs_fenrun_jour_real_detail
	where dt = 'dtonly';
